...
2. Move pure worker/personality agents first.
   - `spark-worker` has no specialized local plugin dependency and can move into `sdlc` or `agent-foundations` as a canonical compiled execution agent.
