...
1. **Task literal** — `worker.model: "gpt-5.6-terra"` wins outright. Source: `task`. After `prism workflow refresh-harness-types`, that string is checked against the installed harness's discovered slugs (Amp `--mode` is `low | medium | high | ultra` plus plugin mode keys). Modelspaces stay optional policy, not the inventory.
