...
1. **Task literal** — `worker.model: "gpt-5.6-terra"` wins outright. Source: `task`. After `prism workflow refresh-harness-types`, that string is checked against the installed harness's discovered slugs. Amp inventories three surfaces: the `--mode` dial (`low | medium | high | ultra`), plugin mode keys, and the curated `provider/model` list from `amp plugins show-agent-options --json` (labels, providers, efforts land in `discovered.json`). Modelspaces stay optional policy, not the inventory.
