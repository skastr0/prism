...
export const astToJsonSchema = (
  ...
  options: AstToJsonSchemaOptions = {},
  fieldPath: string = "",
  visiting: Set<SchemaAST.AST> = new Set(),
): JsonSchema => {
  switch (ast._tag) {
    case "StringKeyword":
      return { type: "string" };
    ...
    case "Union": {
      ...
      if (nonUndefined.length === 1) {
        return astToJsonSchema(nonUndefined[0]!, options, fieldPath, visiting);
      }
      ...
      if (nullLiteral !== undefined && nonNull.length === 1) {
        const item = astToJsonSchema(nonNull[0]!, options, fieldPath, visiting);
    ...
    case "TupleType": {
      ...
      if (tupleAst.elements.length === 0 && tupleAst.rest.length === 1) {
        return { type: "array", items: astToJsonSchema(tupleAst.rest[0]!.type, options, fieldPath, visiting) };
      }
      return unsupportedConstruct(
        "TupleType",
        fieldPath,
    ...
    case "TypeLiteral": {
      ...
      for (const prop of typeLiteralAst.propertySignatures) {
        ...
        const property = astToJsonSchema(prop.type, options, childPath, visiting);
    ...
    case "Suspend": {
      ...
      visiting.add(ast);
      try {
        return astToJsonSchema(ast.f(), options, fieldPath, visiting);
      } finally {
        visiting.delete(ast);
      }
    }
