...
test("claude-code lowerer owner-qualifies foreign tool bindings and merges owner MCP server", async () => {
  ...
  const ownerMcpPath = join(outputRoot, "skills", ownerPluginId, ".mcp.json");
  await writeText(
    ownerMcpPath,
    JSON.stringify(
      {
...
test("claude-code lowerer fails closed when owner MCP config is missing", async () => {
  const root = await createTempRoot();
  const outputRoot = join(root, ".claude");
