...
test("grok lowerer owner-qualifies foreign tool bindings and merges owner MCP server", async () => {
...
test("grok lowerer fails closed when owner MCP config is missing", async () => {
  const root = await createTempRoot();
  const outputRoot = join(root, ".grok");
