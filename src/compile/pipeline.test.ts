...
import { WORKFLOW_REFS_HARNESS, workflowAgentsPath, workflowRefsRoot } from "./workflow-refs-emitter.js";
...
test("canonical TS-authored agents resolve shared toolspace and modelspace bindings", async () => {
  ...
  const workflowRefsPath = workflowAgentsPath(projectRoot);
  expect(await pathExists(workflowRefsPath)).toBe(true);
  const workflowRefs = await readFile(workflowRefsPath, "utf8");
  ...
  expect(workflowRefsSnapshot.manifest.entries.some((entry) => entry.targetPath === workflowRefsPath)).toBe(true);
});
...
test("compilePluginForTarget can auto-select a Hermes HTTP MCP port before config write", async () => {
  const tokenEnv = "PRISM_MCP_COMPILE_AUTO_PORT_TOKEN";
  const { pluginRoot, hermesRoot } = await createHermesHttpToolPlugin({
...
test("compilePluginForTarget verifies an existing auto-port Hermes HTTP MCP daemon", async () => {
  const tokenEnv = "PRISM_MCP_COMPILE_AUTO_PORT_VERIFY_TOKEN";
  const { pluginRoot, hermesRoot } = await createHermesHttpToolPlugin({
        ...
        runtime: {
          mcp: {
            hermes: {
              transport: "streamable-http",
              host: "127.0.0.1",
              tokenEnv: "PRISM_MCP_NO_TOOLS_TOKEN",
            },
          },
        },
      },
