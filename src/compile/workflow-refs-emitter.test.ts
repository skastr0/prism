...
const typecheckGeneratedRefs = async (options: {
  ...
  try {
    ...
    if (options.sopsSource !== undefined) {
...
describe("workflow refs emitter", () => {
  ...
  test("plans a machine-global, project-keyed generated desired root for the sync engine", () => {
    ...
    expect(desired.files[3]?.targetPath).toBe(workflowOrbitsPath(prismHome, projectKey));
    expect(desired.files[3]?.plugin).toBe(WORKFLOW_REFS_HARNESS);
    expect(desired.files[4]?.targetPath).toBe(workflowSopsPath(prismHome, projectKey));
...
`;
  ...
  test("generated sops module typechecks under the workflow tsconfig and schemas decode at runtime", async () => {
    ...
    await typecheckGeneratedRefs({
      agentsSource: renderWorkflowAgentsModule({ manifest }),
      orbitsSource: renderWorkflowOrbitsModule({ manifest }),
      sopsSource,
      probeSource: `
...
`,
  ...
  test("renders literal canonical tool refs from the compile manifest (deterministic camel keys, no sourcePath)", () => {
    const toolRegistry = registry();
    toolRegistry.tools.set("commit-work", {
      name: "commit-work",
      sourcePath: "/tmp/forge/tools/commit-work.tool.ts",
    ...
    toolRegistry.tools.set("create_glyph", {
      name: "create_glyph",
      sourcePath: "/tmp/forge/tools/create_glyph.tool.ts",
      description: "Create a glyph",
      input: {},
      output: {},
      slots: {},
      handle: async () => ({}),
    ...
    expect(output).toContain('"name":"create_glyph"');
    expect(output).not.toContain("sourcePath");
    expect(output).not.toContain("input");
    expect(output).not.toContain("handle");
    expect(output).toContain('as const satisfies Record<string, Record<string, WorkflowCanonicalToolRef>>');
  });
  ...
  test("renders deterministic grouped tool refs for cross-plugin manifests", () => {
    const base = buildCompileManifestForTarget({
      base: emptyCompileManifest(),
      registry: registry(),
      target: "grok",
    ...
    const crossManifest = {
      ...
      tools: {
        ...
        "forge:commit-work": { plugin: "forge", name: "commit-work" },
        "zz:workspace-run": { plugin: "zz", name: "workspace-run" },
      },
    };
    ...
    expect(output).toContain('"workspaceRun"');
    expect(output).not.toContain("sourcePath");
    // plugins grouped deterministically (core before forge before zz)
    const coreIdx = output.indexOf('"core"');
    const forgeIdx = output.indexOf('"forge"');
    const zzIdx = output.indexOf('"zz"');
    expect(coreIdx).toBeGreaterThan(-1);
    expect(forgeIdx).toBeGreaterThan(coreIdx);
    expect(zzIdx).toBeGreaterThan(forgeIdx);
  });

  test("manifest.tools yields source-free deterministic tools ref output", () => {
    const toolRegistry = registry();
    toolRegistry.tools.set("create_glyph", {
      name: "create_glyph",
      sourcePath: "/tmp/forge/tools/create_glyph.tool.ts",
    ...
    const withTools = buildCompileManifestForTarget({
      base: emptyCompileManifest(),
      registry: toolRegistry,
      target: "grok",
      scope: "project",
    ...
    expect(Object.keys(withTools.tools || {}).length).toBeGreaterThan(0);
    const output = renderWorkflowToolsModule({ manifest: withTools });
