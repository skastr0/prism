...
describe("workflow refs emitter", () => {
  ...
  test("plans a machine-global, project-keyed generated desired root for the sync engine", () => {
    ...
    expect(desired.files).toHaveLength(5);
    ...
    expect(desired.files[3]?.targetPath).toBe(workflowSopsPath(prismHome, projectKey));
    expect(desired.files[3]?.plugin).toBe(WORKFLOW_REFS_HARNESS);
    expect(desired.files[4]?.targetPath).toBe(workflowToolsPath(prismHome, projectKey));
