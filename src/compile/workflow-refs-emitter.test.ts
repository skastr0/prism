...
describe("workflow refs emitter", () => {
  ...
  test("plans a machine-global, project-keyed generated desired root for the sync engine", () => {
    ...
    expect(desired.files).toHaveLength(7);
    ...
    expect(desired.files[5]?.targetPath).toBe(workflowSopsPath(prismHome, projectKey));
    expect(desired.files[5]?.plugin).toBe(WORKFLOW_REFS_HARNESS);
    expect(desired.files[6]?.targetPath).toBe(workflowToolsPath(prismHome, projectKey));
    expect(desired.files[6]?.plugin).toBe(WORKFLOW_REFS_HARNESS);
