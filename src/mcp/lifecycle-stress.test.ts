...
const rogueHealthServerScript = (options: {
  ...
  readonly returnedSourceSha256?: string;
  ...
  // This hand-written script does not go through generateMcpServerBundle, so
  // it cannot embed a real PRISM_MCP_SERVER_SOURCE_SHA constant. Default the
  // reported value to whatever the current prism build would expect, so
  // these fixtures exercise only the dimension each test names (pid, sha256)
  // rather than tripping the unrelated runtime-source-freshness check too —
  // unless a test explicitly overrides it to simulate a stale daemon.
...
${sha256Line}
const returnedSourceSha256 = ${JSON.stringify(options.returnedSourceSha256 ?? mcpServerRuntimeSourceSha256())};
...
const server = createServer((req, res) => {
  if (req.url === "/healthz") {
    ...
    res.end(JSON.stringify({
      ...
      serverSourceSha256: returnedSourceSha256,
    }));
    return;
  }
  res.writeHead(404);
...
test("MCP lifecycle status reports stale-build for a running daemon whose bundle predates current runtime source", async () => {
  // Regression coverage for PQ-170: a daemon can be pid-alive, listening, and
  // otherwise healthy (as the incident daemon was) while its bundle was
  // generated by runtime templates that have since changed — nothing about
  // its own plugin's bindings needs to differ for this to happen. The daemon
  // self-reports the runtime-source fingerprint it was built with via
  // /healthz; a mismatch against what the currently-running prism build
  // would generate must classify as stale-build, not "running".
  const fixture = await createFixture();
  const serverName = `prism-generated-${fixture.pluginName}`;
  ...
  await fixture.writeServerScript(
    rogueHealthServerScript({
      serverName,
      returnedPid: "self",
      returnedSourceSha256: "a".repeat(64),
    }),
  );
  ...
  try {
    ...
    const status = await getMcpStatus(fixture.statusOptions());
    expect(status.state).toBe("stale-build");
    expect(status.staleReasons).toContain("health-server-source-sha256-mismatch");
    expect(status.health?.serverSourceSha256).toBe("a".repeat(64));
    expect(formatMcpStatus(status)).toContain("reasons=");
    expect(formatMcpStatus(status)).toContain("health-server-source-sha256-mismatch");
  } finally {
    child.kill("SIGTERM");
    await waitForPidExit(child.pid!, 2_000).catch(() => undefined);
  }
...
test("MCP lifecycle status reports stale-build for a running daemon whose bundle predates the runtime-source fingerprint feature entirely", async () => {
  // A daemon built before PRISM_MCP_SERVER_SOURCE_SHA existed at all cannot
  // report any value for it — that absence is itself unambiguous evidence
  // the daemon predates current source, not a benign "unknown" state.
  const fixture = await createFixture();
  const serverName = `prism-generated-${fixture.pluginName}`;

  await fixture.writeServerScript(`import { createServer } from "node:http";

const host = process.env.PRISM_MCP_HTTP_HOST ?? "127.0.0.1";
const port = Number(process.env.PRISM_MCP_HTTP_PORT ?? "0");
const serverName = process.env.PRISM_MCP_SERVER_NAME ?? ${JSON.stringify(serverName)};
const returnedSha256 = process.env.PRISM_MCP_SERVER_SHA256 ?? "0".repeat(64);
...
`);
  ...
  try {
    ...
    const status = await getMcpStatus(fixture.statusOptions());
    expect(status.state).toBe("stale-build");
    expect(status.staleReasons).toContain("missing-health-server-source-sha256");
    expect(status.health?.serverSourceSha256).toBeUndefined();
  } finally {
    child.kill("SIGTERM");
    await waitForPidExit(child.pid!, 2_000).catch(() => undefined);
  }
