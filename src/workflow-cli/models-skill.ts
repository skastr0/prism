...
- \`amp-orb\` \`worker.project\` is required and typed against the projects \`prism workflow refresh-harness-types\` discovered from \`amp projects list --json\` (namespace/name, owner/repo, or repository URL); validation lists the known ones. \`amp-runner\` \`worker.runnerId\` narrows to the live runners \`prism workflow refresh-harness-types --discover-amp-runners\` captured from the account (one small Amp turn, thread deleted; a plain refresh preserves the capture), and \`worker.runnerDir\` narrows to the directories that runner serves; validation fails closed with both lists. Prefer a named worker that pins either target once.
