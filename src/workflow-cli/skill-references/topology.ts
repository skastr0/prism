...
The worker set is \`claude-code\`, \`opencode\`, \`opencode2\`, \`hermes\`, \`codex-cli\`, \`antigravity-cli\`, \`kimi-code\`, \`amp-code\`, \`cursor\`, \`omp\`, \`grok\`, \`devin\`. An unpinned worker omits the harness \`--model\` flag, so the user's harness default stays in force. Stated preferences (\`prism workflow models --offer\` / \`prefer\`, stored at \`~/.prism/state/workflow-model-preferences.json\`) are copied into \`worker.model\` by the authoring agent — they are never applied at run time.
`;
