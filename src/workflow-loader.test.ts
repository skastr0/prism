...
`);
  ...
  test("validate fails with the allowed worker id list for an unknown worker (WDX-009)", async () => {
    ...
    await expect(validateWorkflowFile(file)).rejects.toThrow(
      /unsupported workflow worker 'not-a-real-worker'\. Supported workers: amp-code, amp-orb, amp-runner, antigravity-cli, claude-code, codex-cli, cursor, devin, grok, hermes, kimi-code, omp, opencode/,
    );
  });
  ...
  test("CLI rejects unsupported workflow workers from the adapter registry", async () => {
    ...
    expect(stderr).toContain("unsupported workflow worker 'not-real'. Supported workers: amp-code, amp-orb, amp-runner, antigravity-cli, claude-code, codex-cli, cursor, devin, grok, hermes, kimi-code, omp, opencode");
  });
